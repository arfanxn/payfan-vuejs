<template>
    <div class>
                    ...
                    <div class="ms-3 text-secondary w-100">
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="fw-bold my-auto m-0 p-0">
                                ...
                                <p class="text-secondary" v-if="order.status?.toUpperCase()?.includes(`PENDING`)">{{
        ...
        StrHelper.make(order.amount).toUSD(false).get()
                                ...
                                <p class="text-success" v-else-if="(order.type?.toUpperCase()?.includes(`REQUESTING`)
        ...
        StrHelper.make(order.amount).toUSD(false).get()
                                ...
                                <p class="text-danger" v-else-if="order.status?.toUpperCase()?.includes(`FAIL`)
        ...
        StrHelper.make(order.amount).toUSD(false).get()
                        ...
                        <div class="d-flex justify-content-between mt-2">
                            <div class>
                                ...
                                <button v-if="(order.type?.toUpperCase()?.includes(`SEND`))" @click.stop="handleSendPayment({
                                    amount: order.amount,
...
&& !(order.status.toUpperCase().includes(`PENDING`))"
                                ...
                                <div v-else-if="(order.type?.toUpperCase()?.includes(`REQUESTED`)) &&
                                (order.status?.toUpperCase()?.includes(`PENDING`))"
                                    class="d-flex justify-content-between">
                                    <button @click.stop="approveRequestMoney(order)"
                                        class="btn btn-outline-primary d-flex rounded-pill fw-bold px-3 text-center py-0">
                                        <small class="my-auto">Approve request</small>
                                    </button>
                                    <button @click.stop="rejectRequestMoney(order)"
                                ...
                                <small v-else-if="(order.type?.toUpperCase()?.includes(`REQUESTED`)) &&
                                (order.status?.toUpperCase()?.includes(`COMPLETE`))"
                                ...
                                <small v-else-if="(order.status?.toUpperCase()?.includes(`FAIL`))"
                                    class="my-auto fw-bold text-center py-0 text-danger">Order or transaction is
                                    Failed</small>
                            </div>
                        ...
                        <!--  activity details -->
                        <ActivityDetails v-if="(currentShowedActivityDetailsID == order.id)" :order="order" />
                    </div>
                </div>
                <!--  -->
            </div>
